...
const SubTaskItem: React.FC<SubTaskItemProps> = ({
  ...
  const handlePress = () => {
    ...
    const completesTask =
      nextCompleted &&
      allSteps.filter((s) => s.id !== step.id).every((s) => s.completed);
  ...
  return (
    ...
    >
      ...
      <Text
        className={`text-base ${step.completed ? "text-muted-foreground line-through decoration-muted-foreground/50" : "text-foreground"}`}
      >
        {step.title}
      </Text>
    </Pressable>
...
const TaskDetailsScreen: React.FC<ITaskDetailsScreenProps> = () => {
  ...
  const toggleSubTask = (taskId: string, subTaskId: string) => {
    ...
    const allCompleted =
      ...
      updatedSteps.every((step: any) => step.completed);
    const nextStatus =
      updatedSteps.length > 0
        ? allCompleted
          ? "Completed"
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
        ...
        <View className="h-64 bg-secondary/40 relative overflow-hidden justify-end pb-8 px-6">
          ...
          <View className="gap-2.5">
            <Badge
              variant="outline"
              className={`self-start ${catStyle.bg} ${catStyle.border} px-3 py-1`}
            >
              <Text
                className={`${catStyle.text} text-[10px] font-black uppercase tracking-widest`}
              >
        ...
        <View className="px-6 py-6 rounded-t-[40px] bg-background -mt-6 flex-1 min-h-[600px]">
          ...
          <View className="flex-row items-center gap-4 bg-secondary/30 border border-border/5 rounded-[28px] p-4 mb-6">
            ...
            <View className="flex-1 flex-row items-center gap-3">
              <View
                className={`h-10 w-10 ${isCompleted ? "bg-emerald-500/10" : "bg-amber-500/10"} rounded-full items-center justify-center`}
              >
                <CheckCircle2
                  size={18}
              ...
              <View>
                ...
                <Text
                  className={`text-sm font-semibold mt-0.5 ${isCompleted ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}
                >
                  {status || "Pending"}
                </Text>
              </View>
          ...
          <View className="mb-6 bg-card border border-border/40 rounded-[28px] p-5 shadow-sm shadow-black/5">
            ...
            <Text className="text-muted-foreground text-base leading-7">
              {description ||
                "No detailed briefing was provided for this task. It represents a focal activity within your checklist queue designed to contribute towards your general weekly progress."}
            </Text>
          </View>
